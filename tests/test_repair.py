...
from interop.repair.validate import (
    RepairAction,
    RepairReport,
    ToolValidator,
    ValidationIssue,
...
from interop.types import CanonicalTool, ToolCall
...
class TestToolValidator:
    def test_valid_call(self):
        ...
        call = ToolCall(id="tc1", name="read_file", arguments={"path": "/tmp/test.txt"})
    ...
    def test_missing_required_arg(self):
        validator = ToolValidator(TOOLS)
        call = ToolCall(id="tc1", name="read_file", arguments={})
        result = validator.validate(call)
    ...
    def test_tool_not_found(self):
        validator = ToolValidator(TOOLS)
        call = ToolCall(id="tc1", name="nonexistent_tool", arguments={"x": "y"})
        result = validator.validate(call)
    ...
    def test_empty_tool_name(self):
        validator = ToolValidator(TOOLS)
        call = ToolCall(id="tc1", name="", arguments={})
        result = validator.validate(call)
        assert not result.valid
    ...
    def test_type_coercion_string(self):
        ...
        validator = ToolValidator(TOOLS)
        call = ToolCall(id="tc1", name="read_file", arguments={"path": 123})
        result = validator.validate(call)
    ...
    def test_arguments_not_dict(self):
        validator = ToolValidator(TOOLS)
        call = ToolCall(id="tc1", name="read_file", arguments="not a dict")  # type: ignore
        result = validator.validate(call)
        assert not result.valid
    ...
    def test_fuzzy_match_tool_name(self):
        ...
        call2 = ToolCall(id="tc2", name="read_file", arguments={"path": "/tmp/x"})
...
class TestRepairToolCalls:
    def test_repair_report_all_valid(self):
        calls = [
            ...
            ToolCall(id="tc2", name="read_file", arguments={"path": "/tmp/b.txt"}),
        ]
    ...
    def test_repair_report_with_invalid(self):
        calls = [
            ...
            ToolCall(id="tc2", name="does_not_exist", arguments={}),
        ]
        report = repair_tool_calls(calls, TOOLS)
        assert report.total_calls == 2
