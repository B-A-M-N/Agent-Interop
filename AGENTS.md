...
- **Compatibility planning**: Runtime inspection, context/tool-surface planning, bounded attempt ladders, digest-scoped bootstrap qualification, and qualified controller routing are implemented. Older known pageable results can be safely reduced (line-preserving text or structure-aware JSON); unknown tools remain verbatim. The controller may request bounded, private primary-worker refinements; those control calls are intercepted by Interop and never exposed to the coding client.
