...
class CapabilityState(str, Enum):
    ...
    def is_available(self) -> bool:
        ...
        ``DECLARED`` is intentionally excluded: it means only that model
        metadata claims the capability — an unverified claim. Counting it
        as available would contradict this module's purpose of distinguishing
        metadata claims from verified results. Only ``PROBED`` (backend
        inspection confirms), ``VERIFIED`` (conformance test passed), and
        ``USER_FORCED`` (user explicitly enabled) qualify.
        """
        return self in (CapabilityState.VERIFIED, CapabilityState.PROBED,
                        CapabilityState.USER_FORCED)
...
class ToolCapabilityLevel(str, Enum):
    ...
    def __lt__(self, other: str) -> bool:  # type: ignore[override]
        if not isinstance(other, ToolCapabilityLevel):
            return NotImplemented
        return self._ORDER.index(str(self.value)) < self._ORDER.index(str(other.value))

    def __le__(self, other: str) -> bool:  # type: ignore[override]
        return self == other or (isinstance(other, ToolCapabilityLevel) and self < other)
...
class AgentCapabilityLevel(str, Enum):
    ...
    def __lt__(self, other: str) -> bool:  # type: ignore[override]
        if not isinstance(other, AgentCapabilityLevel):
            return NotImplemented
        return self._ORDER.index(str(self.value)) < self._ORDER.index(str(other.value))

    def __le__(self, other: str) -> bool:  # type: ignore[override]
        return self == other or (isinstance(other, AgentCapabilityLevel) and self < other)
