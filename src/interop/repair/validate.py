...
from interop.types import CanonicalTool, RepairAction, ToolCall
...
class ToolValidator:
    ...
    def validate(self, call: ToolCall) -> ValidationResult:
        ...
        schema_issues = self._validate_schema(call.arguments, tool.parameters)
    ...
    def _attempt_schema_repair(
        self, call: ToolCall, tool: CanonicalTool, issues: list[ValidationIssue]
    ) -> None:
        ...
        for req in required:
            ...
            if req in props and props[req].get("type") == "string":
                ...
                call.repair = RepairAction.SCHEMA_MINOR
...
def repair_tool_calls(
    calls: list[ToolCall], tools: list[CanonicalTool],
    config: RepairConfig | None = None,
) -> RepairReport:
    """Validate and repair all tool calls in a response.

    Applies repair limits from RepairConfig — caps attempts and latency.
    """
